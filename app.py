...
attempts_used = min(st.session_state.attempts, attempt_limit)
st.progress(
    attempts_used / attempt_limit,
    text=f"Attempts used: {attempts_used}/{attempt_limit}",
)

st.markdown("**Guess History**")
if st.session_state.history:
    st.caption("  •  ".join(str(guess) for guess in st.session_state.history))
else:
...
if st.session_state.status != "playing":
    if st.session_state.status == "won":
        st.success("🎉 You won!")
        score_col, secret_col = st.columns(2)
        with score_col:
